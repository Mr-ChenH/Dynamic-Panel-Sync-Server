...
To run the published image with PostgreSQL, persistent object storage, and the backup worker, prepare `.env` and the mode-`0600` `.compose.env` from their example files, then use the registry-backed Compose file:
...
`compose.image.yml` defaults to `latest`. Set `DP_SYNC_IMAGE` in `.compose.env` to a version tag or digest to pin a production deployment.
...
4. Run `npm run migrate` as a one-shot deployment step, or let Compose run its restricted `database-setup` gate.
